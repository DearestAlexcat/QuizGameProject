...
public class DataCreator_Window : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        data.allSectionData = new Section[0];
        questionsProp = serializedObject.FindProperty("data").FindPropertyRelative("allSectionData");
    }
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(headerRect, "Data to json Creator", headerStyle);

        Rect summaryRect = new Rect(headerRect.x + 25, (headerRect.y + headerRect.height) - 20, headerRect.width - 50, 15);
        GUI.Label(summaryRect, "Create the data that needs to be included into the json file");
        ...
        var arraySize = data.allSectionData.Length;
        ...
        if (pressed)
        {
            ...
            {
                path = EditorUtility.SaveFilePanel("Save", Application.persistentDataPath, "data.json", "json");
            }
            if (string.IsNullOrEmpty(path) == false)
            {
                DataController.SaveData(data, path);
        ...
        if (pressed)
        {
            path = EditorUtility.OpenFilePanel("Select", Application.persistentDataPath, "json");

            data = DataController.LoadData(path);
            serializedObject.ApplyModifiedProperties();
