...
public class ScrollController : MonoBehaviour
{
    ...
    void SwitchRaycasts()
    {
        ...
        if (left >= 0)
        {
            scrollableItems[left].bt.interactable = false;
        }

        if (right < scrollableItems.Count)
        {
            scrollableItems[right].bt.interactable = false;
        }
        
        scrollableItems[current].bt.interactable = true;
    }

    void Update()
    {
        if(scrollableItems.Count == 0)
        {
            return;
        }
